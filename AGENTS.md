...
- **Radio:** single option circle — same label row rhythm as **Checkbox**; compose in **RadioGroup** ([Astryx RadioList](https://astryx.atmeta.com/components/RadioList)). See **ADR-0006**.
- **RadioGroup:** mutually exclusive options — `value` / `onValueChange`, vertical or horizontal layout, group validation band.
- **Switch:** instant on/off — pill track + sliding thumb; `layout="settings"` for label-left rows ([Astryx Switch](https://astryx.atmeta.com/components/Switch)). See **ADR-0006**.
