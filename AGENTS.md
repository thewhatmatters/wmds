...
- **TaskRows:** expandable task rows — composes **Accordion** for disclosure; adds status (**Badge**, **Status**), **meta**, detail rails, and **TaskRows.Detail**. Patterns: **status rows**; **capsules**; optional **Card.Body**.
...
- **Composition audit:** `src/lib/compositionAudit.ts` + **Foundation → Composition audit** — static scan of molecules/organisms for hand-rolled affordances. CI: `npm run validate:composition`.
