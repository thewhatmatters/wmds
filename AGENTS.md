...
- **Mobile nav (examples):** **MobileNavDock** — expandable bottom pill + vertical menu stack for long labels (not a radial arch or icon-only tab bar). **AppShellMobileLayout** maps settings NavList data into one responsive **Tab** row so page content remains visible. See **ADR-0021**, **ADR-0022**; copy **Examples/App shell → Pattern — shell navigation (mobile)**.
