...
- **Card:** [Astryx Card](https://astryx.atmeta.com/components/Card) — `Card.Header`, `Card.Body`, `Card.Footer`. Layout cards (`padding="none"`) use a **white shell + muted inset Body** (`bg-body`); header/footer sit on the shell. **`Card.Body` is a composition slot** — TaskRows, List, forms, or custom canvases. Default **`shape="rounded"`** (`rounded-2xl shadow-md`). **`shape="flush"`** only when a parent owns outer chrome. Simple cards: `padding="md"`.
