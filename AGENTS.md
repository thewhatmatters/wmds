...
- **PageHeader:** horizontal page chrome — `start` | `title` | `end`; **`variant="app"`** (56px canvas band + hairline), **`page`** (section `h2`), **`toolbar`** (`role="toolbar"`, cluster row). Pair **SegmentedControl** / **Button** / **Avatar** at one cluster tier. Not **Card.Header**. **`pageHeaderAppBandHeightClasses`** aligns shell nav bands. See **ADR-0018**.
- **NavList:** sectioned secondary nav — **`sections`** + **`activeId`** / **`onSelect`**, or **NavList.Section** + **NavList.Item**. Inset pills via **Button** `layout="nav"` + **ButtonIcon**; optional counts; **`labelAlign="brand"`** on first section for 56px shell band. Not primary rail. See **ADR-0019**.
...
- **AppShell:** exported application chrome — `items` + `activeId` + `onSelect`; optional `footerItems`, `secondaryNav`, brand, and rail surface. Desktop composes **NavRail** + optional **NavList** + inset canvas; page content goes in **`AppShell.Body`** below **PageHeader**. **`AppShell.Mobile`** maps the same data to an expandable bottom dock and responsive **Tab** row. Routing stays in the app. See **ADR-0023**.
...
- **Mobile nav:** **`AppShell.Mobile`** owns the expandable bottom pill + vertical menu stack for long labels (not a radial arch or icon-only tab bar), and maps secondary NavList data into one responsive **Tab** row so page content remains visible. See **ADR-0021**, **ADR-0022**, **ADR-0023**; copy **Organisms/AppShell → Pattern — mobile navigation**.
