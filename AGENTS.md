...
- **Chart:** composes **[@visx/visx](https://airbnb.io/visx/)** primitives — peer dep, not bundled. WMDS owns shell, tooltip, legend, and **`chartTheme.ts`** token maps; import granular `@visx/*` inside Chart for tree-shaking. See **ADR-0012**, **ADR-0013**, **ADR-0014**. **`Chart.SegmentedBar`** — **capacity meter only** (`value`/`max`, 5px ticks, fills card width); daily history → **Chart.Area** (planned). **Color tiers:** semantic (`ChartTone`), RAG (`fill="semantic"`), categorical (`chartSeriesColor(index)`). **Occupancy KPI:** **Card.Header** `end` → **Select** for period (KPI scope); legend on Cartesian charts in **Card.Body**.
