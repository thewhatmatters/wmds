...
Consuming apps paste **Components/Layout/HeroTileStack → Pattern — marketing hero**. The hero is full width: a title-case display headline (`We Are WhatMatters`, plain `h1`, no uppercase transform), `type-body` subtext with two inline **Badge**s (the one sanctioned decorative use; not clickable; the sentence still reads in order), the stack, and a primary **Button**. Replace the placeholder `src` values with product images. Keep `alt` on every tile.
